...
//  Grid Dash
//
//  Created by Jacob Barth on 11/27/18.
...
class ViewController5: UIViewController {
    ...
    func setPuzzle() {
        setGrid()
        puzzle.append(sum(i: grid[0], j: grid[1], k: grid[2]))
        puzzle.append(sum(i: grid[3], j: grid[4], k: grid[5]))
        puzzle.append(sum(i: grid[6], j: grid[7], k: grid[8]))
        
    }
    func sum(i:String, j:String, k:String) -> Int {
        var ret = 0
    ...
    func check() -> Int {
        var count = 0
        let sum0 = sum(i: grid[0], j: grid[1], k: grid[2])
        let sum1 = sum(i: grid[3], j: grid[4], k: grid[5])
        let sum2 = sum(i: grid[6], j: grid[7], k: grid[8])
    ...
    func title() {
        ...
        r0.text = String(puzzle[0])
        r1.text = String(puzzle[1])
        r2.text = String(puzzle[2])
